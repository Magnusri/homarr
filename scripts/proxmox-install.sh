...
# Modified for Magnusri/homarr fork
...
function update_script() {
  ...
  if check_for_gh_release "homarr" "Magnusri/homarr"; then
...
EOF
    ...
    NODE_VERSION=$(curl -s https://raw.githubusercontent.com/Magnusri/homarr/dev/package.json | jq -r '.engines.node | split(">=")[1] | split(".")[0]')
    setup_nodejs
    CLEAN_INSTALL=1 fetch_and_deploy_gh_release "homarr" "Magnusri/homarr" "prebuild" "latest" "/opt/homarr" "build-debian-amd64.tar.gz"
